...
dl_handle_link_wan(void)
{
	...
	if (dl_status_wan_old != dl_status_wan) {
		...
		if (front_led_x == 1)
			...
			LED_CONTROL(LED_LAN, (dl_status_lan || dl_status_wan) ? LED_ON : LED_OFF);
...
dl_handle_link_lan(void)
{
	...
	if (dl_status_lan_old != dl_status_lan) {
		...
		if (front_led_lan == 2)
			LED_CONTROL(LED_LAN, (dl_status_lan) ? LED_ON : LED_OFF);
...
dl_handle_link_usb(int force_update)
{
	...
	if (dl_status_usb_old != dl_status_usb || force_update) {
		...
		LED_CONTROL(LED_USB2, (dl_status_usb & 0x2) ? LED_ON : LED_OFF);
		LED_CONTROL(LED_USB, (dl_status_usb & 0x1) ? LED_ON : LED_OFF);
	}
}
#endif
...
dl_update_leds(void)
{
	...
	if (front_led_x == 3 && !(search_gpio_led() & LED_USB2)) {
		...
		gpio_led_trig_set(LED_USB, "usb-host");
	} else {
		gpio_led_trig_set(LED_USB, "none");
